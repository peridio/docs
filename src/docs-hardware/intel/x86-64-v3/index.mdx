...
<HardwareHero
  vendor="Intel"
  model="Intel x86-64-v3"
  tagline="High-performance x86-64 microarchitecture level 3 target optimized for AVX2-capable Intel and AMD hardware."
  stats={[
    { value: 'x86-64-v3', label: 'Instruction Set' },
    { value: 'AVX2', label: 'Optimized SIMD' },
    { value: 'Edge AI & Compute', label: 'Deployment Target' },
    { value: 'Haswell+', label: 'Intel Support' },
    { value: 'Excavator+', label: 'AMD Support' },
    { value: 'UEFI', label: 'Boot Mode' },
  ]}
...
Immutable read-only erofs root filesystem with A/B partitioning. Nothing rewrites the running system in place, and an update that fails leaves the active slot untouched.
