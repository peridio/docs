---
title: 'Fast-Track Your AGX Orin Production Deployment'
sidebar_position: 0
description: 'Production-ready NVIDIA Jetson AGX Orin deployment from day 1 with Avocado Connect + Avocado OS. Enterprise Linux, secure OTA, and fleet management for the most powerful edge AI platform.'
hide_title: true
---

import HardwareHero from '@site/src/components/HardwareHero'

<HardwareHero
  vendor="NVIDIA"
  vendorUrl="https://www.nvidia.com/en-us/autonomous-machines/embedded-systems/jetson-agx-orin/"
  model="Jetson AGX Orin"
  tagline="Production-grade software for the most powerful edge AI platform. Deploy NVIDIA Jetson AGX Orin fleets from day 1 with deterministic Linux, secure OTA, and fleet management."
  image="/img/hardware/nvidia/agx-orin.png"
  imageAlt="NVIDIA Jetson AGX Orin module"
  stats={[
    { value: 'Day 1', label: 'Production Ready' },
    { value: '10+ Years', label: 'Long-Term Support' },
    { value: '275 TOPS', label: 'AI Performance' },
    { value: '2048-core', label: 'Ampere GPU' },
    { value: 'Up to 64GB', label: 'LPDDR5 Memory' },
    { value: '12-core', label: 'Cortex-A78AE CPU' },
  ]}
  target="jetson-agx-orin-devkit"
  ctas={{
    primary: {
      label: 'Get Started',
      to: '/developer-reference/getting-started/any-target#jetson-agx-orin-devkit',
    },
  }}
/>

## Overview

The NVIDIA Jetson AGX Orin represents the pinnacle of edge AI computing, delivering unprecedented performance with up to 275 TOPS of AI compute in a compact, energy-efficient package. As NVIDIA's flagship edge platform, the AGX Orin combines exceptional processing power with enterprise-grade reliability, making it ideal for the most demanding autonomous systems, industrial AI, and robotics applications. When paired with Avocado Linux and Avocado Connect, this powerhouse platform gains the production-ready software foundation required for mission-critical deployments at scale. Our enterprise Linux distribution, atomic OTA updates, and comprehensive fleet management unlock the AGX Orin's full potential while dramatically reducing time-to-market. From autonomous vehicles and surgical robots to smart city infrastructure and aerospace applications, teams can focus on breakthrough AI innovation while we handle the complex system integration, security, and lifecycle management. The result is a robust, scalable edge AI solution that matches NVIDIA's hardware excellence with enterprise-grade software infrastructure, backed by 10+ years of support and seamless compatibility across the entire Jetson ecosystem.

## Specifications

| Specification  | Value                              | Notes                                                |
| -------------- | ---------------------------------- | ---------------------------------------------------- |
| AI Performance | 275 TOPS                           | 275 Sparse TOPS, 138 Dense TOPS                      |
| GPU            | 2048-core NVIDIA Ampere GPU        | 64 Tensor Cores for maximum inference throughput     |
| CPU            | 12-core Arm Cortex-A78AE @ 2.2 GHz | Armv8.2 64-bit with advanced safety features         |
| Memory         | 32GB/64GB LPDDR5                   | 204 GB/s bandwidth for complex multi-modal AI        |
| Power          | 15-60W                             | Configurable power modes for any deployment scenario |
| Storage        | 64GB eUFS + NVMe                   | High-speed storage for large AI models and datasets  |

## Use Cases

### Autonomous Vehicles

275 TOPS powers full self-driving stacks with real-time sensor fusion, path planning, and decision making for Level 4+ autonomy.

### Surgical Robotics

Ultra-low latency AI inference for precision surgical procedures. Real-time computer vision and haptic feedback systems.

### Smart City Infrastructure

Process hundreds of camera feeds for traffic optimization, crowd analysis, and emergency response with enterprise reliability.

## Challenges and Solutions

| Challenge                                 | Solution                       |
| ----------------------------------------- | ------------------------------ |
| Developer kits aren't enterprise-ready    | Enterprise-ready OS deployment |
| Complex BSP integration and customization | Pre-validated AGX Orin BSPs    |
| Large-scale OTA orchestration complexity  | Scalable OTA infrastructure    |
| High-availability system requirements     | High-availability architecture |
| Multi-tier fleet management at scale      | Multi-tier fleet orchestration |

## Key Features

### Enterprise Deployment

Deploy production-grade Linux on AGX Orin in minutes. Enterprise toolchain reduces complex system integration time.

### Maximum Performance

Unleash all 275 TOPS with optimized BSPs. Scale across the entire Jetson family with pin-compatibility.

### Enterprise Fleet Management

Manage thousands of AGX Orin devices with advanced targeting, A/B deployments, and comprehensive observability.

### High-Availability Architecture

Built for mission-critical systems with redundancy, failover, and recovery mechanisms for 24/7 operations.

### Ecosystem Compatibility

Seamless integration across NVIDIA ecosystem (Isaac, DeepStream, Triton) and cloud platforms.

### Enterprise Support

10+ years of maintenance with enterprise SLAs. Combined with Jetson's industrial lifecycle for maximum ROI.

## Security features

The published Jetson feed builds encrypted `/var` for every Jetson target; it is opted into per runtime in `avocado.yaml`, with no distro rebuild.

| Capability          | Status                                                                                                                |
| ------------------- | --------------------------------------------------------------------------------------------------------------------- |
| Encrypted `/var`    | Built. LUKS2 key sealed to the OP-TEE fTPM against PCR 7, with an Argon2id fallback                                   |
| Recovery key        | Built. Operator-held master, per-unit passphrase enrolled with `avocado var-key enroll`                               |
| Extension dm-verity | Built. `extensions.<name>.image.verity` is manifest-carried, so it works here too                                     |
| Signed boot FIT     | Not applicable — Jetson boots the NVIDIA chain, not a U-Boot FIT                                                      |
| Rootfs dm-verity    | Not available — the root hash travels in a boot FIT, and the Jetson stone manifests carry no per-slot hash partitions |

```yaml title="avocado.yaml"
runtimes:
  prod:
    var:
      encrypt: true
      hardware: tpm2
      recovery: fleet-var-master
```

The first boot encrypts the flashed `/var` in place, so seeded content — subvolumes, `var_files`, primed Docker images — survives. See [Encrypted /var](/developer-reference/security/encrypted-var).

## Getting Started

### Init, Install, & Build

Follow the [Any Supported Target](/developer-reference/getting-started/any-target#jetson-agx-orin-devkit) instructions under Getting Started to begin. This target is `jetson-agx-orin-devkit`. The provisioning specifics are below.

### Provision

Build the project and execute the provisioning procedure. This will build the system image and flash it to your target hardware.

```bash title="Host machine"
avocado build
avocado provision -r dev --profile tegraflash
```

### Run

Power-cycle the device. It will boot with the provisioned system. The `root` user is passwordless in the `dev` runtime used by this guide.

---

Ready to build? Head to the [Jetson AGX Orin DevKit getting started guide](/developer-reference/getting-started/any-target#jetson-agx-orin-devkit).
