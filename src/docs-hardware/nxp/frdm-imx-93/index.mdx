...
The published i.MX 93 feed builds every security capability this SoC can deliver; each one is opted into per runtime in `avocado.yaml`, with no distro rebuild.

| Capability           | Status on `imx93-frdm`                                                                |
| -------------------- | ------------------------------------------------------------------------------------- |
| Encrypted `/var`     | Built. Key derived from the SoC UID today; the EdgeLock (ELE) backend is pending      |
| Signed boot FIT      | Built. U-Boot re-packed with your key, and a saved-environment permit list            |
...
`imx91-frdm` carries the AHAB wiring too; it does not declare encrypted `/var` or verified boot yet.
