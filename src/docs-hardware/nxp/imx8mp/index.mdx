...
<HardwareHero
  vendor="NXP"
  vendorUrl="https://www.nxp.com/products/processors-and-microcontrollers/arm-processors/i-mx-applications-processors/i-mx-8-applications-processors/i-mx-8m-plus-arm-cortex-a53-machine-learning-vision-multimedia-and-industrial-iot:IMX8MPLUS"
  model="NXP i.MX 8M Plus"
  tagline="Production-ready industrial IoT and vision systems with 2.3 TOPS NPU, dual cameras, and real-time processing."
  image="/img/nxp_iMX8M_Plus.png"
  imageAlt="NXP i.MX 8M Plus development board"
  stats={[
    { value: '2.3 TOPS', label: 'NPU Performance' },
    { value: 'Dual MIPI CSI', label: 'Camera Inputs' },
    { value: '-40 to 85°C', label: 'Industrial Temperature' },
    { value: 'Quad Cortex-A53', label: '2.0 GHz Application CPUs' },
    { value: 'Cortex-M7', label: '800 MHz Real-Time Core' },
    { value: 'Dual GbE + TSN', label: 'Industrial Networking' },
  ]}
  target="imx8mp-evk"
  ctas={{
    primary: {
    ...
    secondary: {
      ...
      to: 'https://blog.peridio.com/hubfs/Hardware%20Solution%20Briefs/NXP%20iMX%208MP.pdf',
      external: true,
    },
  }}
/>
...
The published i.MX 8M Plus feed builds every security capability this SoC can deliver; each one is opted into per runtime in `avocado.yaml`, with no distro rebuild.

| Capability            | Status on `imx8mp-evk`                                                |
| --------------------- | --------------------------------------------------------------------- |
| Encrypted `/var`      | Built. Key bound to a CAAM black key, stored in the LUKS2 header      |
| Signed boot FIT       | Built. U-Boot re-packed with your key, so the first flash enforces it |
| dm-verity rootfs      | Built. Root hash carried in the signed FIT                            |
| dm-verity extensions  | Built                                                                 |
| Bootloader OTA (eMMC) | Built. `boot0`/`boot1` coupled to the OS slots, with rollback         |
...
See [Security features](/developer-reference/security) for the full workflow, including the operator-held [`/var` recovery key](/developer-reference/security/encrypted-var#operator-recovery-key) and [boot signing](/developer-reference/security/boot-signing).
