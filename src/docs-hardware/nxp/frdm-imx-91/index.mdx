...
<HardwareHero
  vendor="NXP"
  vendorUrl="https://www.nxp.com/design/design-center/development-boards-and-designs/FRDM-IMX91"
  model="NXP FRDM i.MX 91"
  tagline="Low-power single Cortex-A55 platform with EdgeLock secure enclave, Wi-Fi 6, and dual Gigabit Ethernet for secure IoT and industrial edge applications."
  image="/img/hardware/nxp/FRDM-IMX91.png"
  imageAlt="NXP FRDM i.MX 91"
  stats={[
    { value: '1.4 GHz', label: 'Cortex-A55' },
    { value: '1GB', label: 'LPDDR4 Memory' },
    { value: 'Wi-Fi 6 + BLE 5.4', label: 'Wireless Connectivity' },
    { value: '8GB eMMC 5.1', label: 'On-Board Storage' },
    { value: '2x GbE + TSN', label: 'Industrial Networking' },
    { value: 'EdgeLock', label: 'Secure Enclave' },
  ]}
  target="imx91-frdm"
  ctas={{
    primary: {
...
| Capability           | Status on `imx91-frdm`                                                                    |
| -------------------- | ----------------------------------------------------------------------------------------- |
| AHAB boot container  | Built as a feed feature: EdgeLock authenticates SPL, ATF and U-Boot against SRK fuses     |
| dm-verity extensions | Built. `extensions.<name>.image.verity` is carried in the runtime manifest                |
| Encrypted `/var`     | Not declared yet — the machine does not list `encrypted-var` in its capabilities          |
| Signed boot FIT      | Not declared yet — this board boots with `booti`, so there is no FIT to sign              |
| Rootfs dm-verity     | Hash partitions are staged in the stone manifest, but need the FIT to carry the root hash |

AHAB is applied when the feed is built rather than from `avocado.yaml`, because it signs the boot container the ROM loads. Signing does not close the part — an OPEN device boots a signed image without checking it, which is the window to validate in before burning SRK fuses. See [Boot signing](/developer-reference/security/boot-signing#ahab-signing-the-imx-9-boot-container).

The sibling [FRDM i.MX 93](/hardware/nxp/frdm-imx-93) carries the same AHAB wiring plus encrypted `/var`, verified boot and rootfs verity.
