...
Follow the [Any Supported Target](/developer-reference/getting-started/any-target) instructions under Getting Started to begin. The MIC-733-AO uses the `jetson-agx-orin` target, with a board selected per SKU:

| SKU           | `AVOCADO_TARGET`  | `AVOCADO_TARGET_BOARD` |
| ------------- | ----------------- | ---------------------- |
| MIC-733-AO5A1 | `jetson-agx-orin` | `mic-733-ao5a1`        |
| MIC-733-AO6A1 | `jetson-agx-orin` | `mic-733-ao6a1`        |

Set these in your `avocado.yaml`, for example:

```yaml title="avocado.yaml"
default_target: 'jetson-agx-orin'
