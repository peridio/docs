---
title: 'Apr 27 - May 3, 2026'
description: 'Avocado CLI updates for the week of Apr 27, 2026.'
---
...
- The rootfs/initramfs overlay snippet now sources the SDK environment itself, so `rsync` (provided by the SDK toolchain) is on `PATH` regardless of how the install command was invoked. Fixes the `bash: rsync: command not found` error when applying an overlay during rootfs or initramfs install.
- `overlay` is now part of the singleton-vs-named-map discriminator for `rootfs:` and `initramfs:` blocks. Previously, combining `overlay:` with `packages:` or `filesystem:` in the singleton form was rejected with `cannot mix singleton form … and named-map form …`.
- Kernel sysroot images are now staged by versioned suffix (e.g. `bzImage-<kver>`) instead of a hardcoded `Image-<kver>` glob, so x86-64 builds stage correctly alongside ARM64. Non-bootable kernel-base artifacts (`System.map-*`, `config-*`) are excluded, and uncompressed images are preferred for the stable `Image` symlink.
