...
- **Target suggestions are filtered to the support-matrix selectable set**, so
  the server only offers targets a user can actually build for (a short two-char
  match like `x86_64` no longer pulls in `qemuarm64`), with a regression test
  locking the guard in place.
