...
Watch the full CLI walkthrough, from `avocado init` to `pipeline_complete`: [watch it on YouTube](https://youtu.be/HPNN8JvsjCQ).

Avocado Desktop runs the same fleet check from inside the app: build the reference, then open the embedded VM shell and watch the pipeline over the in-app terminal, no external terminal needed: [watch it on YouTube](https://youtu.be/Xt5aIkAEYjE).

## What broke, what's deferred

- **The QEMU boot needs an SDK whose `vm` helper pads to a power of two.** This reference's image is ~1.1 GiB. The `2024/edge` SDK's `vm` helper historically resized the disk to a fixed 1 GiB, so `avocado sdk run -iE vm dev` aborted at `qemu-img resize` ("Use the --shrink option") on any image over 1 GiB. The current helper pads to the next power of two instead, which fixes it; if your boot hits that abort, update the SDK (`avocado sdk install`). This affects only the local QEMU boot, not the build or a hardware deploy.
