...
<TestStatus
  targets={[
    '[ENGINEER: build target machine — e.g. qemuarm64, or the platform the Chromium image was built for]',
  ]}
  reference={{
...
<PullQuote>
  Chromium, from scratch: **[ENGINEER: Xh Ym] on one machine → [ENGINEER: Xh Ym] on two** —
  [ENGINEER: N]× on the package that dominates our compile budget.
</PullQuote>
