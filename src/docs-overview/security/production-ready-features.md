...
**How Avocado addresses this:** Partially, and the gaps are worth naming. Signing-key management through the CLI ships today, but build-time generation of a per-machine UEFI key chain is not in the shipping build, and enforcement of the trusted boot chain from silicon through application code is not active on any target (see [Secure Boot](/avocado-os/security/secure-boot)). The read-only root contributes to self-protection: system code cannot be rewritten through the filesystem, though privileged code can still write the backing block device, and read-time block verification is not enabled. Security domain separation and audit are not met today. sysext and confext compose files into a merged root; they do not isolate one extension's processes from another's, which is a job for service-level sandboxing. Journals live on the writable `/var` partition with neither journal sealing nor remote log shipping, so they are not tamper-evident. Meeting these two requirements needs process sandboxing, restrictions on raw block-device access, and sealed or remote logging that the platform does not provide.
...
**How Avocado addresses this:** LUKS encryption provides data at rest protection for the writable var partition where application data lives, under a single `/var` volume key covering all extensions and application data — per-application encryption domains are not implemented (see [Hardware-Backed Encryption](/avocado-os/security/encryption)). Cryptographic erasure is not available: the Argon2id keyslot is derived from the device's SoC identifier and stays enrolled, so destroying a TPM seal does not render `/var` unrecoverable, and there is no key hierarchy or key-management flow to revoke through. Erasing personal data means reformatting the partition. Extension composition is not an access-control boundary between application components either. Systemd journal logging provides audit trail capabilities.

## Runtime security

### Immutable system core

The read-only root filesystem prevents runtime modification of system binaries, libraries, and configuration through the filesystem. Even with root access in production mode, the erofs root cannot be written to at a path. What that does not stop is a write below the filesystem: privileged code that opens the backing block device can modify the image, and until read-time block verification lands nothing catches it. Combined with signature verification at install time, this means the system running in the field is the system that was built and signed, short of the storage being modified underneath the mount.

### Extension isolation

System extensions overlay onto the immutable root. Each extension is independently updatable and can be independently signed at build time; read-time verification per extension is not enabled yet (see [Filesystem Integrity](/avocado-os/security/filesystem-integrity)). Be precise about what the overlay gives you: extension images and the merged root are read-only, so no extension rewrites another one's files in place. It is a composition mechanism, not a process boundary. A compromised service shipped in an extension runs with whatever privileges it was granted, and can reach `/var`, other services, and the backing block devices accordingly. Confining it is a job for service-level sandboxing, not for sysext.
