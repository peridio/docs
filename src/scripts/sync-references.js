...
// ── Configuration ──────────────────────────────────────────────────────────────
const REFERENCES_DIR = path.resolve(__dirname, '..', '..', '..', 'avocado-os-references')
...
const GITHUB_BASE = 'https://github.com/avocado-linux/avocado-os-references/tree/main'
...
function parseReference(name, dirPath) {
  ...
  if (frontmatter.targets) {
    if (frontmatter.targets === '*' || (Array.isArray(frontmatter.targets) && frontmatter.targets.includes('*'))) {
      targets = ['*']
...
function main() {
  ...
  if (!fs.existsSync(REFERENCES_DIR)) {
    console.error(`Error: references directory not found at ${REFERENCES_DIR}`)
    console.error('Expected the avocado-os-references repo to be checked out alongside the docs repo.')
    process.exit(1)
  }
  ...
  for (const entry of entries) {
    ...
    const ref = parseReference(entry.name, path.join(REFERENCES_DIR, entry.name))
