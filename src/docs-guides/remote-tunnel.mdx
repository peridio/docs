...
Remote tunnels are a feature of [Avocado Connect](/avocado-connect/overview), the fleet management and device operations platform for Avocado OS. The device dials out and Connect's relay brokers the connection, so you can reach hardware behind NAT, firewalls, or cellular links with no static IPs, port forwarding, or VPN setup. The device-to-relay leg is encrypted with **WireGuard**; traffic between your machine and the relay endpoint is forwarded as-is, carrying only the encryption the tunneled service itself provides (SSH encrypts itself, plain HTTP does not). Every session is audit-logged — who connected, when, and for how long.
...
- **Web ports (80/443/9090/custom)**: open the tunnel endpoint in your browser — `http://<host:port>` for plain HTTP, `https://<host:port>` for HTTPS and Cockpit. On the TLS ports, expect a browser certificate warning: the certificate identifies the device, not the relay address (and Cockpit's default certificate is self-signed). For a device hosting a frontend on port `3000` or `5000`, you see exactly what a local user sees; for Cockpit on `9090`, you get the device's management UI.

### Example: view a device dashboard on a custom port

First, close the SSH tunnel from the previous section (**Close** next to the tunnel) — the developer plan and the default per-device tunnel policy allow one active tunnel at a time.
...
:::warning
This dashboard is plain HTTP: traffic between your machine and the relay is unencrypted, and the endpoint is reachable by anyone your organization's tunnel-policy CIDR allowlist permits (`0.0.0.0/0` by default) until the tunnel closes. Tighten the allowlist for sensitive services.
:::
...
- Availability and limits (TTL, concurrency, CIDR allowlist, allowed windows) are governed by your organization's tunnel policy — see the [Remote Access API](/developer-reference/avocado-connect-api/remote-access) for details.
- For a higher-level look at remote access in Connect, see the [Remote Access overview](/avocado-connect/tunnels).
