...
:::caution Not yet proven on hardware

This page documents `avocado-set-boot-device`, which is written and unit-tested but has **not been run on a board**. It is held as a draft until it has. The mechanism it relies on - writing UEFI `BootOrder` on a Jetson - was confirmed by hand on an Orin Nano: the write persisted across reboot and `BootCurrent` followed it.

NVIDIA Jetson (Tegra) is the only target this page covers.
...
Current boot order, first entry wins:
  ...
  Boot0000* UEFI Shell  FvVol(...)/FvFile(...)
  Boot0002* UEFI Network  MAC(...)/IPv4(0.0.0.0)
...
The first block is the boot order as the firmware holds it, first entry winning. The second maps the device classes you can ask for onto the entries that match. A dash means no entry on this board refers to a device of that class.
...
If you are switching boot devices on a board with more than one provisioned disk, check `findmnt -no SOURCE /` after the reboot. That is the authoritative answer to which disk you are actually running from.
