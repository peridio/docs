...
- `targets`: (list of strings) A list of binary target strings. When peridiod receives a bundle, it will install only binaries with target strings in this list. Defaults to `["portable"]`. Use `[]` to accept all.
