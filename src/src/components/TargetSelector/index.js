...
const targetList = Object.values(targets).sort((a, b) =>
    ...
    <div>
      <div className={styles.selectorBox}>
        <div className={styles.selectorHeader}>
          ...
          <div>
            <h3 className={styles.selectorTitle}>Select your target</h3>
            <p className={styles.selectorDescription}>
              Choose your hardware target to see the complete getting started guide.
              See the <Link to="/hardware/support-matrix"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" width="14" height="14" style={{verticalAlign: 'middle', marginRight: '3px', marginTop: '-2px'}}><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 3v1.5M4.5 8.25H3m18 0h-1.5M4.5 12H3m18 0h-1.5m-15 3.75H3m18 0h-1.5M8.25 19.5V21M12 3v1.5m0 15V21m3.75-18v1.5m0 15V21m-9-1.5h10.5a2.25 2.25 0 002.25-2.25V6.75a2.25 2.25 0 00-2.25-2.25H6.75A2.25 2.25 0 004.5 6.75v10.5a2.25 2.25 0 002.25 2.25zm.75-12h9v9h-9v-9z" /></svg> hardware support matrix</Link> for all supported targets.
            </p>
          </div>
        </div>
      ...
      {t && (
        <div className={styles.targetInfo}>
          ...
          <ul>
            ...
            <li>The latest version of the <Link to="/developer-reference/avocado-cli/overview">Avocado CLI</Link></li>
          </ul>

          {t.provisioning.warnings.includes('linuxHostOnly') && (
            <div className={styles.warningBox}>
              ⚠ {WARNING_MESSAGES.linuxHostOnly}
            </div>
          )}
          ...
          {t.serial && (
            <>
              ...
              <p>
                ...
                <a href="https://www.amazon.com/dp/B07WX2DSVB">this or similar</a>
                ) to your target. The adapter must be set to {t.serial.voltage}.
              </p>
              {t.serial.gpio && (
                <>
                  ...
                  <ul>
                    {t.serial.gpio.filter(p => p.note === 'recovery mode').length > 0 && (
                      <li>Boot into recovery mode:
                        <ul>
                          {t.serial.gpio.filter(p => p.note === 'recovery mode').map((pin, i) => (
                            <li key={i}>
                              <span style={{
                                display: 'inline-block', width: '14px', height: '14px',
                                backgroundColor: pin.color, borderRadius: '2px',
                    ...
                    <li>Connect your serial console adapter:
                      <ul>
                        {t.serial.gpio.filter(p => p.note !== 'recovery mode').map((pin, i) => (
                          <li key={i}>
                            <span style={{
              ...
              <pre><code>{t.serial.command}</code></pre>
              <p>Replace <code>/dev/ttyUSB0</code> with the appropriate device path for your adapter.</p>
            </>
          )}

          {t.provisioning.recoveryMode && (
            <>
              <h2>Boot into Recovery Mode</h2>
              ...
              <pre><code>{t.provisioning.recoveryMode.verifyCommand}</code></pre>
          ...
          <pre><code>{`avocado init --target ${t.target} ${t.target}\ncd ${t.target}`}</code></pre>

          <h2>Install</h2>
          <p>Install the SDK toolchain, extension dependencies, and runtime packages.</p>
          <pre><code>avocado install -f</code></pre>

          <h2>Build</h2>
          <p>Build the system image.</p>
          <pre><code>avocado build</code></pre>

          <h2>Provision</h2>
          {t.provisioning.provisionDescription ? (
            <p>{t.provisioning.provisionDescription}</p>
          ) : t.provisioning.profile ? (
            <p>Provision the <code>dev</code> runtime using the <code>{t.provisioning.profile}</code> profile:</p>
          ) : (
            <p>Provision the <code>dev</code> runtime:</p>
          )}
          <pre><code>{t.provisioning.provisionCommand}</code></pre>
          {t.provisioning.provisionSteps && t.provisioning.provisionSteps.map((step, i) => (
            step.type === 'code' ? (
              <pre key={i}><code>{step.content}</code></pre>
            ) : (
          ...
          {t.category === 'virtual' ? (
            <>
              ...
              <pre><code>avocado sdk run -iE vm dev</code></pre>
            </>
          ...
          {t.provisioning.bootNote ? (
            ...
            <p>The <code>root</code> user is passwordless in the <code>dev</code> runtime.</p>
          )}

          <h2>SSH Access</h2>
          <p>Once booted, SSH into the device:</p>
          <pre><code>{'ssh root@<device-ip>'}</code></pre>
